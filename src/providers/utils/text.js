...
var CoreTextUtilsProvider = /** @class */ (function () {
    function CoreTextUtilsProvider(translate, langProvider, modalCtrl, sanitizer) {
     ...
     * @return Treated text.
     */
    CoreTextUtilsProvider.prototype.addEndingSlash = function (text) {
        if (!text) {
            return '';
        }
        if (text.slice(-1) != '/') {
     ...
     */
    CoreTextUtilsProvider.prototype.addTextToError = function (error, text) {
     ...
     */
    CoreTextUtilsProvider.prototype.buildSeveralParagraphsMessage = function (paragraphs) {
        var _this = this;
        // Filter invalid messages, and convert them to messages in case they're errors.
        var messages = [];
        paragraphs.forEach(function (paragraph) {
            // If it's an error, get its message.
            var message = _this.getErrorMessageFromError(paragraph);
            if (paragraph) {
     ...
     * @return Escaped text.
     */
    CoreTextUtilsProvider.prototype.escapeHTML = function (text, doubleEncode) {
        if (doubleEncode === void 0) { doubleEncode = true; }
        if (typeof text == 'undefined' || text === null || (typeof text == 'number' && isNaN(text))) {
            return '';
        }
        else if (typeof text != 'string') {
     ...
     */
    CoreTextUtilsProvider.prototype.expandText = function (title, text, component, componentId, files, filter, contextLevel, instanceId, courseId) {
     ...
     */
    CoreTextUtilsProvider.prototype.replaceDraftfileUrls = function (siteUrl, text, files) {
        var _this = this;
        if (!text || !files || !files.length) {
            return { text: text };
        }
        var draftfileUrl = this.concatenatePaths(siteUrl, 'draftfile.php');
        var matches = text.match(new RegExp(this.escapeForRegex(draftfileUrl) + '[^\'" ]+', 'ig'));
        ...
        matches.forEach(function (url) {
            ...
            if (pluginfileMap[filename]) {
                ...
                text = text.replace(new RegExp(_this.escapeForRegex(url), 'g'), pluginfileMap[filename]);
            }
        });
        return {
            text: text,
     ...
     * @return Treated text.
     */
    CoreTextUtilsProvider.prototype.restoreDraftfileUrls = function (siteUrl, treatedText, originalText, files) {
        var _this = this;
        if (!treatedText || !files || !files.length) {
            return treatedText;
        }
        var draftfileUrl = this.concatenatePaths(siteUrl, 'draftfile.php');
        var draftfileUrlRegexPrefix = this.escapeForRegex(draftfileUrl) + '/[^/]+/[^/]+/[^/]+/[^/]+/';
        files.forEach(function (file) {
            // Search the draftfile URL in the original text.
            var matches = originalText.match(new RegExp(draftfileUrlRegexPrefix + _this.escapeForRegex(file.filename) + '[^\'" ]*', 'i'));
            if (!matches || !matches[0]) {
                return; // Original URL not found, skip.
            }
            treatedText = treatedText.replace(new RegExp(_this.escapeForRegex(file.fileurl), 'g'), matches[0]);
        });
        return treatedText;
    };
    /**
     ...
     */
    CoreTextUtilsProvider.prototype.substrReplace = function (str, replace, start, length) {
     ...
     * @param courseId Course ID the text belongs to. It can be used to improve performance with filters.
     */
    CoreTextUtilsProvider.prototype.viewText = function (title, text, options) {
        if (text.length > 0) {
            options = options || {};
            var params = {
                title: title,
            ...
            var modal = this.modalCtrl.create('CoreViewerTextPage', params, options.modalOptions);
            modal.present();
        }
    };
...
export { CoreTextUtilsProvider };
var CoreTextUtils = /** @class */ (function (_super) {
    __extends(CoreTextUtils, _super);
    function CoreTextUtils() {
        return _super !== null && _super.apply(this, arguments) || this;
    }
    return CoreTextUtils;
}(makeSingleton(CoreTextUtilsProvider)));
export { CoreTextUtils };
